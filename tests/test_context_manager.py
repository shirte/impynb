...
def test_context_manager_function() -> None:
    ...
    notebook_finder._skip_cell_tags = original_tags.copy()

    # Test the context manager
    test_tags = ["test", "debug"]
    with configure(skip_cell_tags=test_tags):
        # Within context, should have test tags
        assert notebook_finder._skip_cell_tags == test_tags

    # After context, should be restored to original
    assert notebook_finder._skip_cell_tags == original_tags
...
def test_context_manager_nested() -> None:
    ...
    with configure(skip_cell_tags=outer_tags):
        assert notebook_finder._skip_cell_tags == outer_tags

        with configure(skip_cell_tags=inner_tags):
            assert notebook_finder._skip_cell_tags == inner_tags
...
def test_context_manager_default_empty() -> None:
    ...
    with configure():  # No arguments, should use empty list
        assert notebook_finder._skip_cell_tags == []
...
def test_context_manager_none_input() -> None:
    ...
    with configure(skip_cell_tags=None):  # None should be converted to empty list
        assert notebook_finder._skip_cell_tags == []
